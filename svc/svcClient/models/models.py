...
class Sprint(db.Entity):
    ...
    issue_status = Required(Json)  # 问题状态: fixing: '待办, 进行中, 新发现', fixed: '开发完成', verified: '已关闭'
    issue_categories = Required(StrArray, default=['regression', 'previous', 'newfeature', 'others'])  # 问题类别标签列表
    queries = Optional(Json)  # jql 集
...
class IssueSprint(db.Entity):
    ...
    categories = Required(Json)  # 缺陷类别: regression, previous, code_change, others
    found_since = Required(Json)  # 缺陷发现来源: new_feature, regression_improve, qa_missed
    found_in_rcs = Required(Json)  # 缺陷发现版本: rc1, rc2, rc3, rc4, rc5
